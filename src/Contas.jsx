...
const Contas = ({
  ...
  const salvarOuRessalvar = () => {
    ...
    const payload = {
      ...
      paga: idInterno ? contasCadastradas.find((c) => c.id === idInterno)?.paga ?? false : false
    };
      ...
      <Box sx={{ flexGrow: 1, p: 4, display: 'flex', flexDirection: 'column', gap: 3 }}>
        ...
        <Grid container spacing={2}>
          <Grid item xs={12} md={3} onClick={limparCampos}>
            <ActionCard label="NOVA CONTA" icon={<AddCircleOutlineIcon sx={{ color: '#128654', fontSize: '3rem' }} />} />
          </Grid>
          <Grid item xs={12} md={3} onClick={() => prepararLista('EXCLUIR')}>
            <ActionCard label="EXCLUIR CONTA" icon={<HighlightOffIcon sx={{ color: '#C62828', fontSize: '3rem' }} />} />
          </Grid>
          ...
          <Grid item xs={12} md={3} onClick={() => prepararLista('CONSULTAR')}>
            <ActionCard label="CONSULTAR / BAIXA" icon={<ListAltIcon sx={{ color: '#1976D2', fontSize: '3rem' }} />} />
          </Grid>
        </Grid>
        ...
        {modoLista ? (
          <TableContainer component={Paper} sx={{ borderRadius: '25px', maxHeight: '60vh', border: `1px solid ${theme.palette.divider}` }}>
            <Table stickyHeader>
              <TableHead>
                <TableRow>
                  <TableCell sx={{ bgcolor: 'background.paper' }}><strong>Fornecedor</strong></TableCell>
                  <TableCell sx={{ bgcolor: 'background.paper' }}><strong>Vencimento</strong></TableCell>
                  <TableCell sx={{ bgcolor: 'background.paper' }}><strong>Valor</strong></TableCell>
                  <TableCell sx={{ bgcolor: 'background.paper' }}><strong>Status</strong></TableCell>
              ...
              <TableBody>
                {contasCadastradas.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={5} align="center">Nenhuma conta cadastrada.</TableCell>
                  </TableRow>
                ) : (
